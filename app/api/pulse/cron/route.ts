...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.log("[CRON] Previous cache has", prevPHByTitle.size, "PH products");

    const freshTitles = new Set(freshPHSignals.map((s: any) => s.title?.trim()));

    // Live (< 24h): fresh upvotes, preserve claudeGap
    ...
    // Frozen (> 24h): keep as-is, upvotes frozen
    ...
    return NextResponse.json({ ok: true, signals: signals.length, appStore: appSnaps.length, playStore: playSnaps.length, phLive: mergedLivePH.length, phFrozen: frozenPH.length, hasMovementData });
  } catch (err) {
...
async function fetchProductHuntLast24h(fetchHeaders: Record<string, string>): Promise<any[]> {
  ...
  console.log(`[CRON] PH fetching since: ${postedAfter.toISOString()}`);
  ...
  console.log(`[CRON] PH: ${allEdges.length} products (total: ${totalCount ?? "unknown"})`);
...
function detectMovements(current: AppSnapshot[], previous: AppSnapshot[]): any[] {
  ...
  for (const snap of current) {
    ...
    if (prev) {
      ...
      if (prev.rank >= 20 && snap.rank <= 10 && rankDiff >= 10) {
        signals.push({ source: snap.source, sourceLabel, emoji: "\u{1F4C8}", title: snap.app_name, subtitle: `#${prev.rank} → #${snap.rank} in ${snap.category}`, signal: `Jumped from #${prev.rank} to #${snap.rank} in ${snap.category} (▲${rankDiff})`, url: snap.url, timestamp: now, movementType: "rank_jump", prevRank: prev.rank, newRank: snap.rank, rankChange: rankDiff });
      }
      if (prev.review_count && snap.review_count) {
        ...
        if (gained >= 500) {
          ...
          signals.push({ source: snap.source, sourceLabel, emoji: "\u{1F4AC}", title: snap.app_name, subtitle: `${gained.toLocaleString()} new reviews in 1h`, signal: `${gained.toLocaleString()} new reviews (+${pct}%)`, url: snap.url, timestamp: now, movementType: "review_spike", prevRank: prev.rank, newRank: snap.rank, rankChange: prev.rank - snap.rank });
        }
      }
      ...
      signals.push({ source: snap.source, sourceLabel, emoji: "\u{1F195}", title: snap.app_name, subtitle: `New entry at #${snap.rank} in ${snap.category}`, signal: `New entry at #${snap.rank} in ${snap.category}`, url: snap.url, timestamp: now, movementType: "new_entry", newRank: snap.rank, rankChange: 0 });
    }
  }
  if (biggestMover && biggestMover.diff >= 10) {
    ...
    if (!already) {
      const sourceLabel = snap.source === "appstore" ? "App Store" : "Google Play";
      signals.push({ source: snap.source, sourceLabel, emoji: "\u{1F680}", title: snap.app_name, subtitle: `Biggest mover — up ${diff} positions`, signal: `Biggest mover this hour — up ${diff} positions (#${prevRank} → #${snap.rank})`, url: snap.url, timestamp: new Date().toISOString(), movementType: "top_mover", prevRank, newRank: snap.rank, rankChange: diff });
    }
  }
  return signals;
}
...
function detectLongTermMovements(current: AppSnapshot[], previous: AppSnapshot[], period: "weekly" | "monthly"): any[] {
  ...
  for (const snap of current) {
    ...
    signals.push({ source: snap.source, sourceLabel, emoji, title: snap.app_name, subtitle: `${label} #${prev.rank} → now #${snap.rank} in ${snap.category}`, signal: `${label} #${prev.rank} → now #${snap.rank} (${direction}${Math.abs(rankDiff)})`, url: snap.url, timestamp: now, movementType, prevRank: prev.rank, newRank: snap.rank, rankChange: rankDiff });
  }
