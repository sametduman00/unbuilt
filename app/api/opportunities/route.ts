...
const OPPORTUNITY_TYPES = [
  "Momentum — new releases gaining traction fast",
  "Monopoly — category dominated by 1-2 players, room for alternatives",
  "Gap — missing features or niches not served by existing apps",
  "Complaint — low ratings and common complaints in existing apps",
  "Price — expensive incumbents, room for an affordable alternative",
  "Geography — US/English dominated, opportunity in other markets/languages",
  "Bundle — multiple single-purpose apps that could be combined into one",
];
...
Each opportunity must be one of these 7 types:
${OPPORTUNITY_TYPES.map((t, i) => `${i + 1}. ${t}`).join("\n")}
...
[
  {
    ...
    "evidence": "<specific data point from the provided data that supports this>",
...
- "evidence" must reference specific app names, ratings, review counts, or PH vote counts
