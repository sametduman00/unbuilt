...
function buildHtml(report: Report): string {
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>${e(toolLabel)} — ${e(report.idea)}</title><style>${css}</style></head><body><div class="header"><div class="badge-tool">${e(toolLabel)}</div><h1>${e(report.idea)}</h1><div class="meta">${dateStr}</div></div>${sec.join("\n")}<div class="footer">Generated by Unbuilt.me &middot; ${dateStr}</div></body></html>`;
}
...
export default function ReportsPage() {
  ...
  const handlePdf = (report: Report) => {
    ...
    // Create invisible iframe - no new tab, direct download
    ...
    // Load html2pdf inside the iframe then trigger download
