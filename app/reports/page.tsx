...
window.onload = function() { setTimeout(downloadPDF, 800); };
</script><div class="header"><div class="badge-tool">${e(toolLabel)}</div><h1>${e(report.idea)}</h1><div class="meta">${dateStr}</div></div>${sec.join("\n")}<div class="footer">Generated by Unbuilt.me &middot; ${dateStr}</div></body></html>`;
}
...
export default function ReportsPage() {
  ...
  const handlePdf = (report: Report) => {
    ...
    if (!win) {
      ...
      a.href = url;
      a.download = report.idea.replace(/[^a-z0-9]+/gi, "-").toLowerCase() + "-report.html";
      a.click();
    }
