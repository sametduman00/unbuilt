...
export default function PulsePage() {
  ...
  const analyzeNewPHSignals = useCallback(async (currentSignals: Signal[]) => {
    ...
    const unanalyzed = currentSignals.filter((s) => s.source === "producthunt" && !s.claudeGap);
    ...
    try {
      ...
      for (const a of analyses) {
        gapMap.set(a.name?.trim().toLowerCase(), `${a.what} \u2726 Different: ${a.different} \u2726 Missing: ${a.missing}`);
      }
    ...
    } catch { /* silent */ } finally { analyzingRef.current = false; setAnalyzing(false); }
  }, []);
  ...
  const fetchAppStore = useCallback(async () => {
    ...
    } catch { /* silent */ } finally { setAsLoading(false); }
  }, []);
      ...
      <main style={{ maxWidth: 720, margin: "0 auto", padding: "2rem 1.5rem 4rem" }}>
        ...
        {activeTab === "ph" && (
          <div>
              ...
              <h1 style={{ fontSize: "1.75rem", fontWeight: 700, letterSpacing: "-0.03em", margin: 0, display: "flex", alignItems: "center", gap: 10 }}>
                ...
                {!loading && (hasMovementData ? (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: "0.6875rem", fontWeight: 600, padding: "0.2rem 0.6rem", borderRadius: 999, background: "rgba(34,197,94,0.12)", color: "#22c55e", letterSpacing: "0.02em" }}>
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#22c55e", display: "inline-block", animation: "livePulse 2s ease-in-out infinite" }} /> LIVE
                  </span>
                ) : (
            ...
            {error && !loading && (
              <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 10, padding: "1rem 1.25rem", color: "#ef4444", fontSize: "0.875rem" }}>{error}</div>
            )}
            {!loading && !error && signals.length === 0 && (
              <div style={{ textAlign: "center", padding: "3rem 0", color: "var(--clr-text-3)" }}>No signals right now. Check back soon.</div>
            )}
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem", flexWrap: "wrap" }}>
                          <span style={{ fontSize: "0.625rem", fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase", padding: "0.125rem 0.5rem", borderRadius: 999, background: `${sourceColor}18`, color: sourceColor }}>{s.sourceLabel}</span>
                          {s.movementType && s.movementType !== "trending" && s.movementType !== "ph_trending" && (
                            <span style={{ fontSize: "0.5625rem", fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", padding: "0.1rem 0.4rem", borderRadius: 999, background: `${movementColor}20`, color: movementColor }}>
                              {s.movementType === "rank_jump" && "RANK JUMP"}{s.movementType === "new_entry" && "NEW ENTRY"}{s.movementType === "review_spike" && "REVIEW SPIKE"}{s.movementType === "top_mover" && "TOP MOVER"}{s.movementType === "weekly_mover" && "WEEKLY"}{s.movementType === "monthly_mover" && "MONTHLY"}
                            </span>
                          )}
                        ...
                        {isPH && s.topics && s.topics.length > 0 && (
                          <div style={{ display: "flex", gap: "0.25rem", marginTop: "0.25rem", flexWrap: "wrap" }}>
                            {s.topics.map((topic, ti) => (
                              <span key={topic} style={{ fontSize: "0.5625rem", fontWeight: 600, letterSpacing: "0.03em", padding: "0.1rem 0.375rem", borderRadius: 999, background: `${TOPIC_COLORS[ti % TOPIC_COLORS.length]}18`, color: TOPIC_COLORS[ti % TOPIC_COLORS.length] }}>{topic}</span>
                            ))}
                          </div>
                        )}
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "0.375rem", flexWrap: "wrap", marginBottom: "1.25rem" }}>
              {["all","Business","Productivity","Finance","Health & Fitness","Education","Utilities","Developer Tools","Social Networking"].map((cat) => (
                <button key={cat} onClick={() => setAsFilter(cat)} style={{
                  ...
                  borderRadius: 999, padding: "0.25rem 0.75rem", fontSize: "0.75rem",
                  fontWeight: asFilter === cat ? 600 : 400, cursor: "pointer", transition: "all 0.15s",
                }}>
                  {cat === "all" ? "All" : cat}
                </button>
              ))}
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem", flexWrap: "wrap" }}>
                        ...
                        {app.category && (
                          <span style={{ fontSize: "0.5625rem", fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", padding: "0.1rem 0.4rem", borderRadius: 999, background: "rgba(99,102,241,0.12)", color: "#6366f1" }}>{app.category}</span>
                        )}
