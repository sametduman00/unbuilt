...
function parseClaudeGap(gap: string | undefined): { what: string; different: string; missing: string } | null {
  ...
  return {
    ...
    different: parts[1].replace(/^Different:\s*/i, "").trim(),
    missing: parts[2].replace(/^Missing:\s*/i, "").trim(),
  };
}
function getVotes(s: Signal): number {
...
export default function PulsePage() {
  ...
  useEffect(() => {
    fetchSignals();
    const t = setInterval(fetchSignals, 3600000);
  ...
  useEffect(() => {
    if (activeTab === "appstore" && asApps.length === 0) fetchAppStore();
  }, [activeTab, asApps.length, fetchAppStore]);
  ...
  const analyzedCount = phSignals.filter(s => s.claudeGap).length;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--clr-bg)", color: "var(--clr-text)" }}>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                ...
                {!loading && (hasMovementData
                  ...
                  : <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: "0.625rem", fontWeight: 700, padding: "0.2rem 0.6rem", borderRadius: 999, background: "rgba(251,191,36,0.12)", color: "#fbbf24" }}>INITIALIZING</span>
                )}
              </div>
              <p style={{ color: "var(--clr-text-3)", fontSize: "0.875rem", margin: "0 0 0.75rem" }}>
                {phSignals.length > 0 ? `${phSignals.length} products tracked · ${analyzedCount} analyzed · upvotes updated hourly` : "Collecting first snapshot..."}
              </p>
              {allTopics.length > 0 && (
                <div style={{ display: "flex", gap: "0.375rem", flexWrap: "wrap" }}>
                  {["all", ...allTopics.slice(0, 15)].map((t) => (
                    ...
                    <a key={s.title + i} href={s.externalUrl || s.url} target="_blank" rel="noopener noreferrer"
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", marginBottom: "0.5rem", flexWrap: "wrap" }}>
                          {votes > 0 ? (
                            <div style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(218,85,47,0.1)", border: "1px solid rgba(218,85,47,0.2)", borderRadius: 8, padding: "0.25rem 0.625rem" }}>
                              <svg width="11" height="11" viewBox="0 0 24 24" fill="#DA552F"><path d="M12 4l8 8H4z"/></svg>
                              <span style={{ fontSize: "0.875rem", fontWeight: 700, color: "#DA552F" }}>{votes.toLocaleString()}</span>
                              <span style={{ fontSize: "0.75rem", color: "rgba(218,85,47,0.7)" }}>upvotes</span>
                            </div>
                          ) : (
                            <div style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(var(--clr-text-rgb),0.04)", border: "1px solid var(--clr-border)", borderRadius: 8, padding: "0.25rem 0.625rem" }}>
                              <svg width="11" height="11" viewBox="0 0 24 24" fill="var(--clr-text-4)"><path d="M12 4l8 8H4z"/></svg>
                          ...
                          {isLive
                            ...
                            : <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: "0.625rem", fontWeight: 700, padding: "0.2rem 0.5rem", borderRadius: 999, background: "rgba(148,163,184,0.08)", color: "var(--clr-text-4)", letterSpacing: "0.04em" }}>❄ FROZEN</span>
                          }
                        </div>
                        ...
                        {s.topics && s.topics.length > 0 && (
                          <div style={{ display: "flex", gap: "0.25rem", marginBottom: "0.625rem", flexWrap: "wrap" }}>
                            {s.topics.map((t, ti) => (
                              <span key={t} style={{ fontSize: "0.625rem", fontWeight: 600, padding: "0.15rem 0.5rem", borderRadius: 999, background: TOPIC_COLORS[ti % TOPIC_COLORS.length] + "18", color: TOPIC_COLORS[ti % TOPIC_COLORS.length] }}>{t}</span>
                            ))}
                          </div>
                        )}
                          ...
                          <div style={{ background: "rgba(var(--clr-text-rgb),0.03)", border: "1px solid var(--clr-border)", borderRadius: 10, padding: "0.75rem 1rem" }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                              <div style={{ display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
                                <span style={{ fontSize: "0.625rem", fontWeight: 700, color: "#6366f1", background: "rgba(99,102,241,0.12)", padding: "0.1rem 0.45rem", borderRadius: 5, flexShrink: 0, marginTop: 1, whiteSpace: "nowrap" }}>WHAT</span>
                                <span style={{ fontSize: "0.8125rem", color: "var(--clr-text-3)", lineHeight: 1.5 }}>{gap.what}</span>
                              </div>
                              <div style={{ display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
                                <span style={{ fontSize: "0.625rem", fontWeight: 700, color: "#06b6d4", background: "rgba(6,182,212,0.12)", padding: "0.1rem 0.45rem", borderRadius: 5, flexShrink: 0, marginTop: 1, whiteSpace: "nowrap" }}>DIFF</span>
                                <span style={{ fontSize: "0.8125rem", color: "var(--clr-text-3)", lineHeight: 1.5 }}>{gap.different}</span>
                              </div>
                              <div style={{ display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
                                <span style={{ fontSize: "0.625rem", fontWeight: 700, color: "#f59e0b", background: "rgba(245,158,11,0.12)", padding: "0.1rem 0.45rem", borderRadius: 5, flexShrink: 0, marginTop: 1, whiteSpace: "nowrap" }}>GAP</span>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {asApps.filter((app) => asFilter === "all" || app.category === asFilter).map((app) => (
                  <a key={app.app_id} href={app.store_url} target="_blank" rel="noopener noreferrer"
                    style={{ display: "flex", alignItems: "flex-start", gap: "1rem", padding: "1.25rem", background: "var(--clr-surface)", border: "1px solid var(--clr-border)", borderLeft: "3px solid #007AFF", borderRadius: 12, textDecoration: "none", color: "inherit", transition: "background 0.15s" }}
                    onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(var(--clr-text-rgb),0.02)"; }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = "var(--clr-surface)"; }}>
                    {app.icon_url
      ...
      <style>{`@keyframes livePulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.4; transform: scale(0.85); } }`}</style>
    </div>
  );
                                                                                                                                                                                                                                                                                                                                                         }
