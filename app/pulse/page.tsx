...
interface Signal {
  source: string; sourceLabel: string; emoji: string; title: string;
  subtitle: string; signal: string; url: string; timestamp: string;
...
interface AppStoreApp {
  id: number; app_id: string; app_name: string; developer: string;
  category: string; description: string; icon_url: string; store_url: string;
  price: string; release_date: string; first_seen_at: string;
  rating: number | null; review_count: number;
  claude_what: string | null; claude_different: string | null; claude_missing: string | null;
}

const SOURCE_COLORS: Record<string, string> = { appstore: "#007AFF", playstore: "#34A853", producthunt: "#DA552F" };
...
export default function PulsePage() {
  ...
  const analyzeNewPHSignals = useCallback(async (sigs: Signal[]) => {
    ...
    try {
      ...
      setSignals((prev) => prev.map((s) => { if (s.source !== "producthunt" || s.claudeGap) return s; const g = gapMap.get(s.title?.trim().toLowerCase()); return g ? { ...s, claudeGap: g } : s; }));
  ...
  const fetchAppStore = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      setAsApps(data.apps ?? []); setAsGeneratedAt(data.generatedAt ?? null);
  ...
  const CATS = ["all","Business","Productivity","Finance","Health & Fitness","Education","Utilities","Developer Tools","Social Networking"];
      ...
      <main style={{ maxWidth: 720, margin: "0 auto", padding: "2rem 1.5rem 4rem" }}>
        {activeTab === "ph" && (
          <div>
            <div style={{ marginBottom: "1.5rem" }}>
              <h1 style={{ fontSize: "1.75rem", fontWeight: 700, letterSpacing: "-0.03em", margin: 0, display: "flex", alignItems: "center", gap: 10 }}>
                ...
                {!loading && (hasMovementData
                  ? <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: "0.6875rem", fontWeight: 600, padding: "0.2rem 0.6rem", borderRadius: 999, background: "rgba(34,197,94,0.12)", color: "#22c55e", letterSpacing: "0.02em" }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: "#22c55e", display: "inline-block", animation: "livePulse 2s ease-in-out infinite" }} />LIVE</span>
                  : <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: "0.6875rem", fontWeight: 600, padding: "0.2rem 0.6rem", borderRadius: 999, background: "rgba(251,191,36,0.12)", color: "#fbbf24", letterSpacing: "0.02em" }}>INITIALIZING</span>
                )}
              </h1>
              <p style={{ color: "var(--clr-text-3)", fontSize: "0.9375rem", margin: "0.25rem 0 0" }}>{hasMovementData ? "Tracking rank movements, new entries, and review spikes" : "Collecting first snapshot \u2014 movement detection starts next hour"}</p>
              {analyzing && <p style={{ color: "var(--clr-text-4)", fontSize: "0.75rem", margin: "0.25rem 0 0", display: "flex", alignItems: "center", gap: 4 }}><span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: "#DA552F", animation: "livePulse 1.5s ease-in-out infinite" }} />Analyzing new products...</p>}
            </div>
            {loading && (
            ...
            {!loading && !error && signals.length === 0 && <div style={{ textAlign: "center", padding: "3rem 0", color: "var(--clr-text-3)" }}>No signals right now. Check back soon.</div>}
            {!loading && signals.length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {signals.map((s, i) => {
                  const sc = SOURCE_COLORS[s.source] || "#888";
                  const mc = MOVEMENT_COLORS[s.movementType ?? ""] ?? undefined;
                  const showRank = s.rankChange && s.rankChange > 0 && s.movementType !== "trending";
                  const isPH = s.source === "producthunt";
                  return (
                    <a key={s.source + s.movementType + i} href={s.externalUrl || s.url} target="_blank" rel="noopener noreferrer"
                      style={{ display: "flex", alignItems: "flex-start", gap: "0.875rem", padding: "0.875rem 1rem", background: "var(--clr-surface)", border: "1px solid var(--clr-border)", borderLeft: mc ? "3px solid " + mc : "1px solid var(--clr-border)", borderRadius: 10, textDecoration: "none", color: "inherit", transition: "background 0.15s" }}
                      ...
                      {isPH && s.imageUrl ? <img src={s.imageUrl} alt="" width={48} height={48} style={{ borderRadius: 10, flexShrink: 0, objectFit: "cover" }} /> : <span style={{ fontSize: "1.5rem", flexShrink: 0, width: 48, height: 48, display: "flex", alignItems: "center", justifyContent: "center" }}>{s.emoji}</span>}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem", flexWrap: "wrap" }}>
                          <span style={{ fontSize: "0.625rem", fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase", padding: "0.125rem 0.5rem", borderRadius: 999, background: sc + "18", color: sc }}>{s.sourceLabel}</span>
                          {s.movementType && s.movementType !== "trending" && s.movementType !== "ph_trending" && <span style={{ fontSize: "0.5625rem", fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", padding: "0.1rem 0.4rem", borderRadius: 999, background: (mc ?? "") + "20", color: mc }}>{s.movementType === "rank_jump" ? "RANK JUMP" : s.movementType === "new_entry" ? "NEW ENTRY" : s.movementType === "review_spike" ? "REVIEW SPIKE" : s.movementType === "top_mover" ? "TOP MOVER" : s.movementType === "weekly_mover" ? "WEEKLY" : "MONTHLY"}</span>}
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                          ...
                          {showRank && <span style={{ display: "inline-flex", alignItems: "center", gap: 2, fontSize: "0.75rem", fontWeight: 700, color: "#22c55e", flexShrink: 0, padding: "0.125rem 0.4rem", borderRadius: 6, background: "rgba(34,197,94,0.1)" }}><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M5 2L8 6H2L5 2Z" fill="#22c55e" /></svg>{s.rankChange}</span>}
                        </div>
                        {isPH && s.makerName && <div style={{ fontSize: "0.75rem", color: "var(--clr-text-4)", marginTop: "0.125rem" }}>by {s.makerName}</div>}
                        {isPH && s.topics && s.topics.length > 0 && <div style={{ display: "flex", gap: "0.25rem", marginTop: "0.25rem", flexWrap: "wrap" }}>{s.topics.map((t, ti) => <span key={t} style={{ fontSize: "0.5625rem", fontWeight: 600, padding: "0.1rem 0.375rem", borderRadius: 999, background: TOPIC_COLORS[ti % TOPIC_COLORS.length] + "18", color: TOPIC_COLORS[ti % TOPIC_COLORS.length] }}>{t}</span>)}</div>}
                        {s.prevRank && s.newRank && s.movementType !== "trending" && <div style={{ fontSize: "0.8125rem", fontWeight: 600, marginTop: "0.125rem", display: "flex", alignItems: "center", gap: "0.25rem" }}><span style={{ color: "var(--clr-text-4)" }}>#{s.prevRank}</span><span style={{ color: "var(--clr-text-4)", fontSize: "0.75rem" }}>{"\u2192"}</span><span style={{ color: "#22c55e" }}>#{s.newRank}</span></div>}
                        {s.source === "appstore" && s.rating && <div style={{ fontSize: "0.75rem", color: "#f59e0b", marginTop: "0.125rem", display: "flex", alignItems: "center", gap: "0.375rem" }}><span>{renderStars(s.rating)}</span><span style={{ color: "var(--clr-text-4)" }}>{s.rating.toFixed(1)}{s.reviewCount ? " \u2022 " + s.reviewCount.toLocaleString() + " reviews" : ""}</span></div>}
                        <div style={{ fontSize: "0.8125rem", color: "var(--clr-text-3)", marginTop: "0.125rem" }}>{s.signal}</div>
                        {s.claudeGap ? <div style={{ fontSize: "0.75rem", color: "var(--clr-text-4)", marginTop: "0.375rem", fontStyle: "italic", display: "flex", alignItems: "flex-start", gap: "0.25rem" }}><span style={{ flexShrink: 0 }}>{"\u{1F4A1}"}</span><span>{s.claudeGap}</span></div> : isPH && <div style={{ fontSize: "0.75rem", color: "var(--clr-text-4)", marginTop: "0.375rem", fontStyle: "italic", opacity: 0.5 }}>Analyzing...</div>}
                      </div>
        ...
        {activeTab === "appstore" && (
          <div>
            <div style={{ marginBottom: "1.5rem" }}>
              <h1 style={{ fontSize: "1.75rem", fontWeight: 700, letterSpacing: "-0.03em", margin: 0, display: "flex", alignItems: "center", gap: 10 }}>
                {"New & Rising"}
                <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: "0.6875rem", fontWeight: 600, padding: "0.2rem 0.6rem", borderRadius: 999, background: "rgba(0,122,255,0.12)", color: "#007AFF", letterSpacing: "0.02em" }}>APP STORE</span>
              </h1>
              <p style={{ color: "var(--clr-text-3)", fontSize: "0.9375rem", margin: "0.25rem 0 0" }}>New apps from the last 90 days</p>
              {asGeneratedAt && <p style={{ color: "var(--clr-text-4)", fontSize: "0.75rem", margin: "0.25rem 0 0" }}>Updated {relativeTime(asGeneratedAt)}</p>}
            </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {asApps.filter((app) => asFilter === "all" || app.category === asFilter).map((app) => (
                  <a key={app.app_id} href={app.store_url} target="_blank" rel="noopener noreferrer"
                    style={{ display: "flex", alignItems: "flex-start", gap: "0.875rem", padding: "0.875rem 1rem", background: "var(--clr-surface)", border: "1px solid var(--clr-border)", borderLeft: "3px solid #007AFF", borderRadius: 10, textDecoration: "none", color: "inherit", transition: "background 0.15s" }}
                    onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(var(--clr-text-rgb),0.03)"; }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = "var(--clr-surface)"; }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {(app.claude_what || app.claude_different || app.claude_missing) ? <div style={{ fontSize: "0.75rem", color: "var(--clr-text-4)", marginTop: "0.375rem", fontStyle: "italic", display: "flex", alignItems: "flex-start", gap: "0.25rem" }}><span style={{ flexShrink: 0 }}>{"\uD83D\uDCA1"}</span><span>{app.claude_what} \u2726 Different: {app.claude_different} \u2726 Missing: {app.claude_missing}</span></div> : <div style={{ fontSize: "0.75rem", color: "var(--clr-text-4)", marginTop: "0.375rem", fontStyle: "italic", opacity: 0.5 }}>Analyzing...</div>}
                    </div>
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ flexShrink: 0, color: "var(--clr-text-4)", marginTop: 4 }}><path d="M6 3l5 5-5 5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  </a>
                ))}
