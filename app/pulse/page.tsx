...
export default function PulsePage() {
  ...
  const [asGenAt, setAsGenAt] = useState<string|null>(null);
  ...
  const fetchAS = useCallback(async () => {
    setAsLoading(true);
    try { const r = await fetch("/api/pulse/appstore"); const d = await r.json(); setAsApps(d.apps??[]); setAsGenAt(d.generatedAt??null); }
    catch {} finally { setAsLoading(false); }
  }, []);
  useEffect(() => { if (tab === "appstore" && asApps.length === 0) fetchAS(); }, [tab, asApps.length, fetchAS]);
        ...
        {tab === "appstore" && (
          <div>
            <div style={{marginBottom:"1.5rem"}}>
              ...
              {asGenAt && <p style={{color:"var(--clr-text-4)",fontSize:"0.75rem",margin:"0 0 0.75rem"}}>Updated {relativeTime(asGenAt)}</p>}
              <div style={{display:"flex",gap:"0.375rem",flexWrap:"wrap"}}>
                {AS_CATS.map(cat => (
            ...
            {!asLoading && asApps.length > 0 && (
              <div style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>
                {asApps.filter(a => asFilter==="all"||a.category===asFilter).map(app => (
