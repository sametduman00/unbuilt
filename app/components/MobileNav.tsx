          ...
          <div style={{ position:"fixed", top:52, left:0, right:0, background:"var(--clr-surface)", borderBottom:"1px solid var(--clr-border)", zIndex:99, overflowY:"auto", maxHeight:"calc(100vh - 108px)" }}>
            ...
            {isSignedIn ? (
              <>
                ...
                {/* My Reports */}
                {DIV}
                <Link href="/reports" onClick={close} style={{ ...ROW, justifyContent: "flex-start" } as React.CSSProperties}>
                  ...
                  My Reports
                  {plan?.tier === "free" && <span style={{ fontSize: 9, fontWeight: 700, color: "#6366f1", background: "rgba(99,102,241,0.1)", padding: "1px 6px", borderRadius: 4, letterSpacing: "0.04em" }}>PRO</span>}
                </Link>
