...
export default function ConsentGate({ children }: { children: React.ReactNode }) {
  ...
  const [consentGiven, setConsentGiven] = useState(true); // start true to avoid flash
  const [check1, setCheck1] = useState(false); // Terms, Privacy, Disclaimer
  const [check2, setCheck2] = useState(false); // Cookie, Acceptable Use, AI Transparency, Do Not Sell
  ...
  // Once signed in and consent already given, keep open
  useEffect(() => {
    if (isSignedIn && consentGiven) {
      // already good
    }
  }, [isSignedIn, consentGiven]);
  ...
  // Show children immediately if consent already given and signed in
  ...
  return (
    <>
      ...
      {/* Render children underneath (blurred) */}
